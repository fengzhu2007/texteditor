    ...
    while (index < tokens.size()) {
        ...
        switch (token.kind) {
            ...
            case Token::Number:
                ...
                break;
            case Token::Type:
                setFormat(token.offset, token.length, formatForCategory(C_TYPE));
                break;
            case Token::Generic:
                setFormat(token.offset, token.length, formatForCategory(C_PRIMITIVE_TYPE));
                break;
            case Token::Decorator:
                setFormat(token.offset, token.length, formatForCategory(C_LABEL));
                break;
            case Token::Comment:
