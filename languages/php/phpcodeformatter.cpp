...
namespace Php {
    ...
    void CodeFormatter::recalculateStateAfter(const QTextBlock &block,int lexerState,const QString& currentLine,int* tokenIndex){
        ...
        for (; *tokenIndex < m_tokens.size(); ) {
            ...
            case objectdefinition_or_php:
                ...
                default:            turnInto(top_php); continue;
                } break;

            case use_start:
                enter(use_maybe_slash_or_version_or_as);
                break;

            case use_maybe_slash_or_version_or_as:
                switch (kind) {
                case Code::Token::Dot:           turnInto(use_slash); break;
                case As:            turnInto(use_as); break;
                case Code::Token::Number:        turnInto(use_maybe_as); break;
            ...
            case use_slash:
                switch (kind) {
                case Code::Token::Identifier:    turnInto(use_maybe_slash_or_version_or_as); break;
                default:            leave(); leave(); continue;
                } break;
            ...
            default:
                if(pHtmlFormatter!=nullptr){
                    qWarning() << "php Unhandled state" << pHtmlFormatter->m_currentState.top().type;
                }else{
                    qWarning() << "php Unhandled state" << m_currentState.top().type;
                }
                break;
            } // end of state switch
    ...
    void CodeFormatter::enter(int newState)
    {
        ...
        if(pHtmlFormatter!=nullptr){
            ...
            m_newStates.push(s);
            //qDebug() << "php enter state 2" << stateToString(newState)<<m_currentLine.mid(m_currentToken.begin(),m_currentToken.length)<<"indent:"<<m_indentDepth<<"size:"<<pHtmlFormatter->m_currentState.size();
        }
    ...
    void CodeFormatter::leave(bool statementDone)
    {
        ...
        if(pHtmlFormatter!=nullptr){
            ...
            //qDebug() << "php left state2" << stateToString(poppedState.type) << ", now in state" << stateToString(topState)<<m_indentDepth <<"size:"<<m_currentState.size()<<"indent:"<<m_indentDepth;
        }
    ...
    QStack<State> CodeFormatter::initialState()
    {
        ...
        if (initialState.isEmpty())
            initialState.push(State(topmost_intro_php, 0));
        return initialState;
    }
    ...
    {
        qDebug() << "php Current token index" << m_tokenIndex;
        qDebug()<< "php Current state:";
        for (const State &s : pHtmlFormatter->m_currentState) {
            qDebug() << stateToString(s.type) << s.savedIndentDepth;
        }
        qDebug() << "php Current indent depth:" << m_indentDepth;
    }
    ...
    void CodeFormatter::onEnter(int newState, int *indentDepth, int *savedIndentDepth) const
    {
        ...
        case expression_maybe_continuation:
            ...
            for (int i = 1; state(i).type != topmost_intro_php; ++i) {
