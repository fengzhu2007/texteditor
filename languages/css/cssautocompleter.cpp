...
static Code::Token tokenUnderCursor(const QTextCursor &cursor)
{
    ...
    for (; tokenIndex < tokens.size(); ++tokenIndex) {
        ...
        if (token.is(Code::Token::Comment,Code::Token::Css) || token.is(Code::Token::String,Code::Token::Css)) {
                                              ...
                                              const QString &textToInsert) const
{
    ...
    case Code::Token::String: {
        ...
        if (quote != QLatin1Char('"') && quote != QLatin1Char('\'')) {
            ...
            if ((startState & Scanner::MultiLineStringDQuote) == Scanner::MultiLineStringDQuote)
                quote = QLatin1Char('"');
            else if ((startState & Scanner::MultiLineStringSQuote) == Scanner::MultiLineStringSQuote)
                quote = QLatin1Char('\'');
        }
...
{
    return tokenUnderCursor(cursor).is(Code::Token::Comment,Code::Token::Css);
}
