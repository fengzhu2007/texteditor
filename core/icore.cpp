...
{
    return tr("Configure...", "msgShowOptionsDialog");
...
QString ICore::msgShowOptionsDialogToolTip()
{
    if (Utils::HostOsInfo::isMacHost())
        return tr("Open Preferences dialog.", "msgShowOptionsDialogToolTip (mac version)");
    else
        return tr("Open Options dialog.", "msgShowOptionsDialogToolTip (non-mac version)");
}
...
FilePath ICore::resourcePath(const QString &rel)
{
    return FilePath::fromString(
        QDir::cleanPath(QCoreApplication::applicationDirPath() + '/' ) + "/"+rel);
